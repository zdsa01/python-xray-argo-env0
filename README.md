...
本仓库已增加 `env0-helm/`，可作为 env0 的 Helm Template 部署到已连接的 Kubernetes 集群。

部署步骤与变量说明见 `ENV0_DEPLOY.md`。默认只暴露 HTTP 订阅端口 `3000`；Cloudflare Tunnel 使用主动出站连接，不需要对外暴露 `ARGO_PORT`。
