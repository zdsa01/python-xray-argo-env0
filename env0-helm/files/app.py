...
def download_files_and_run():
    ...
    if NEZHA_SERVER and NEZHA_PORT and NEZHA_KEY:
        ...
        command = f"nohup {shlex.quote(str(npm_path))} -s {shlex.quote(f'{NEZHA_SERVER}:{NEZHA_PORT}')} -p {shlex.quote(NEZHA_KEY)} {nezha_tls} --disable-auto-update --report-delay 4 --skip-conn --skip-procs >/dev/null 2>&1 &"
        try:
            exec_cmd(command)
        ...
        command = f'nohup {shlex.quote(str(php_path))} -c {shlex.quote(str(nezha_config_path))} >/dev/null 2>&1 &'
        try:
            exec_cmd(command)
    ...
    if not web_path.exists():
        raise RuntimeError('Xray binary download failed; check outbound network access and download source.')
    command = f'nohup {shlex.quote(str(web_path))} -c {shlex.quote(str(config_path))} >/dev/null 2>&1 &'
    try:
        exec_cmd(command)
    ...
    # 运行 Cloudflared
    if bot_path.exists():
        if ARGO_AUTH and 'TunnelSecret' not in ARGO_AUTH:
            args = f"tunnel --edge-ip-version auto --no-autoupdate --protocol http2 run --token {shlex.quote(ARGO_AUTH)}"
        ...
        try:
            if boot_log_path.exists():
                ...
                if argo_domains:
                    ...
                    time.sleep(3)
                    args = f'tunnel --edge-ip-version auto --no-autoupdate --protocol http2 --logfile {boot_log_path} --loglevel info --url http://localhost:{ARGO_PORT}'
                    exec_cmd(f'nohup {shlex.quote(str(bot_path))} {args} >/dev/null 2>&1 &')
                    log(f'{bot_name} is running')
                    time.sleep(6)
                    extract_domains()
        except Exception as e:
...
class RequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == f'/{SUB_PATH}':
            self.send_response(200 if startup_ready.is_set() else 503)
            self.send_header('Content-Type', 'text/plain; charset=utf-8')
            self.end_headers()
        ...
        if path == '/':
            ...
            self.end_headers()
            self.wfile.write(
                f'Hello world!<br><br>You can access /{SUB_PATH} to get your nodes!'.encode('utf-8'))
            return
...
if __name__ == '__main__':
    ...
    # 在后台线程中启动主流程；致命初始化错误主动退出，让 Kubernetes/容器编排器重启实例
    def bootstrap():
        try:
            start_server()
        except Exception as exc:
            log_error(f'Fatal startup error: {exc}')
            os._exit(1)
